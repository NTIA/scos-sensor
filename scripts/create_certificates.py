...
def sign_csr(csr, ca_public_key, ca_private_key, save_path=None, sans=[]):
    ...
    issuer_subject_key_identifier = ca_public_key.extensions.get_extension_for_class(
    ...
    public_key = builder.sign(private_key=ca_private_key, algorithm=hashes.SHA512())
...
def main():
    ...
    if "ca_private_key_save_path" in settings[ini_section]:
        ...
        if not os.path.isabs(ca_private_key_save_path):
            ca_private_key_save_path = os.path.join(
                os.getcwd(), ca_private_key_save_path
            )
    ...
    if "ca_private_key_path" in settings[ini_section]:
        ...
        if not os.path.isabs(existing_ca_private_key_path):
            existing_ca_private_key_path = os.path.join(
                os.getcwd(), existing_ca_private_key_path
            )
    else:
    ...
    if (existing_ca_private_key_path and not existing_ca_cert_path) or (
        not existing_ca_private_key_path and existing_ca_cert_path
    ):
        print(
            "ca_private_key_path and ca_certificate_path both must be specified in ini file to use existing certificate authority!"
        )
        sys.exit(1)
    ...
    if existing_ca_cert_path and existing_ca_private_key_path:
        ...
        with open(existing_ca_private_key_path, "rb") as ca_key:
            existing_ca_private_key = serialization.load_pem_private_key(
                ca_key.read(), password=None
            )

    if not existing_ca_cert and not existing_ca_private_key:
        if not ca_private_key_save_path:
            raise Exception(
                "Must specify ca_private_key_save_path in ini file when not using existing certificate authority!"
            )
        if not ca_common_name:
            raise Exception(
                "Must specify CA_COMMON_NAME in ini file when not using existing certificate authority!"
            )
        # create a ca private key
        ca_private_key = gen_private_key(
            passphrase=key_passphrase, save_path=ca_private_key_save_path
        )
        ...
        ca_public_key = gen_public_key(
            ...
            sans=[
                x509.SubjectAlternativeName(
                    [
                        x509.DNSName(ca_san_dns),
                        x509.IPAddress(ipaddress.IPv4Address(ca_san_ip)),
                    ]
                )
            ],
            ca=True,
    ...
    server_private_key = gen_private_key(
        passphrase=key_passphrase, save_path=server_private_key_path
    )
    ...
    sign_csr(
        ...
        ca_public_key,
        ca_private_key,
        save_path=server_cert_path,
        sans=[
            x509.SubjectAlternativeName(
                [
                    x509.DNSName(server_san_dns),
                    x509.IPAddress(ipaddress.IPv4Address(server_san_ip)),
                ]
            )
        ],
    )
    ...
    client_private_key = gen_private_key(
        passphrase=key_passphrase, save_path=client_private_key_path
    )
    ...
    sign_csr(
        ...
        ca_public_key,
        ca_private_key,
        save_path=client_cert_path,
        sans=[
            x509.SubjectAlternativeName(
                [
                    x509.DNSName(client_san_dns),
                    x509.IPAddress(ipaddress.IPv4Address(client_san_ip)),
                ]
            )
        ],
    )
