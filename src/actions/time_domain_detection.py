...
class TimeDomainDetection(Action):
    ...
    """
    def __init__(self, detector, frequency, sample_rate, number_of_samples):
        super(TimeDomainDetection, self).__init__()
        self.detector = detector
        self.frequency = frequency
        self.sample_rate = sample_rate
        self.usrp = usrp_iface 
        self.number_of_samples = number_of_samples# make instance variable to allow hotswapping mock
    ...
    def apply_detector(self, data):
        ...
        mean_power_watts = calculate_mean_power(data_scaled) / impedance
        detector_data = 10 * np.log10(mean_power_watts) + 30
    ...
    def archive(self, detector_data, sigmf_md, parent_entry, task_id):
        ...
        Acquisition(
            ...
            data=np.array([detector_data], dtype=np.float32)).save()

    @property
    def description(self):
        return """Apply {} detector on m x {}-sample complex time series acquired at {:.2f} MHz.
