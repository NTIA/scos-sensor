...
def check_power_cycle_settings():
    if settings.SIGAN_POWER_SWITCH is None:
        logger.warning("SIGAN_POWER_SWITCH is not set. SCOS will not be able to power cycle the signal analyzer if needed.")
    if settings.SIGAN_POWER_CYCLE_STATES is None:
        logger.warning("SIGAN_POWER_CYCLE_STATES is not set. SCOS will not be able to power cycle the signal analyzer if needed.")
