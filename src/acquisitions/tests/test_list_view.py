...
def test_non_existent_entry(client, testclock):
    with pytest.raises(AssertionError):
        get_acquisition_list(client, 'doesntexist')


@pytest.mark.django_db
def test_entry_with_no_acquisition_response(client, testclock):
...
@pytest.mark.django_db
def test_single_acquisition_response(client, testclock):
...
@pytest.mark.django_db
def test_multiple_acquisition_response(client, testclock):
...
@pytest.mark.django_db
def test_delete_list(client, testclock):
    entry_name = simulate_acquisitions(client, n=3)
