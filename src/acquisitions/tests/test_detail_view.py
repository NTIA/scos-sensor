...
from acquisitions.tests.utils import (
    get_acquisition_detail, reverse_acquisition_detail, simulate_acquisitions,
    HTTPS_KWARG)
...
@pytest.mark.django_db
def test_non_existent_task_id(client, testclock, test_user):
...
@pytest.mark.django_db
def test_get_detail_from_single(client, testclock, test_user):
    client.login(username=test_user.username, password=test_user.password)
...
@pytest.mark.django_db
def test_get_detail_from_multiple(client, testclock, test_user):
    client.login(username=test_user.username, password=test_user.password)
...
@pytest.mark.django_db
def test_delete_single(client, testclock, test_user):
    client.login(username=test_user.username, password=test_user.password)
