...
def test_token_user_unauthorized(settings, live_server, user):
    client = RequestsClient()
    response = client.get(
        f"{live_server.url}", headers={"Authorization": f"Token {user.auth_token.key}"}
    )
...
def test_bad_token_forbidden(settings, live_server):
    ...
    response = client.get(
        f"{live_server.url}", headers={"Authorization": f"Token {token}"}
    )
    print(f"headers: {response.request.headers}")
    assert response.status_code == 401
...
def test_urls_unauthorized(live_server, user):
    ...
    response = client.get(
        f"{live_server.url}{capabilities}",
    ...
    response = client.get(
        f"{live_server.url}{schedule_list}",
        headers={"Authorization": f"Token {user.auth_token.key}"},
    )
    assert response.status_code == 403
    ...
    response = client.get(
        f"{live_server.url}{task_results_overview}",
        headers={"Authorization": f"Token {user.auth_token.key}"},
    )
    assert response.status_code == 403
    ...
    response = client.get(
        f"{live_server.url}{upcoming_tasks}",
        headers={"Authorization": f"Token {user.auth_token.key}"},
    )
    assert response.status_code == 403
...
def test_user_cannot_view_user_detail_role_change(settings, live_server, alt_user):
    ...
    response = client.get(
        f"{live_server.url}{user_detail}",
        headers={"Authorization": f"Token {alt_user.auth_token.key}"},
    )
    assert response.status_code == 200
...
def test_admin_can_view_other_user_detail(
    settings, live_server, admin_user, alt_admin_user
):
    client = RequestsClient()
