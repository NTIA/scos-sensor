...
def get_oauth_authorized_client(token, live_server):
    ...
    with requests_mock.Mocker(real_http=True) as m:
        _oauth_callback_url = reverse("oauth_callback")
        # m.post(OAUTH_TOKEN_URL+f"?grant_type=authorization_code&code=test_code&client_id={CLIENT_ID}&redirect_uri={live_server.url}{_oauth_callback_url}", json={"access_token": token})
        ...
        #     # json {item.json()}
        #     print(f"url: {item.url} qs: {item.qs}  headers: {item.headers}")
        #     try:
        #         if item.text: print("text: " + str(item.text))
        #     except:
        #         pass
        #     try:
        #         if item.json: print("json: " + str(item.json))
        #     except:
        #         pass
        #     try:
        #         if item.text(): print("text: " + str(item.text()))
