...
class Scheduler(threading.Thread):
    ...
    def __init__(self):
        ...
        preselector = load_preselector(settings.PRESELECTOR_CONFIG, settings.PRESELECTOR_MODULE,
                                       settings.PRESELECTOR_CLASS, capabilities["sensor"])
        location = None
        self.sensor = Sensor(signal_analyzer=sigan, preselector=preselector, switches=switches, capabilities=capabilities,
                        location=location)
        register_sensor.send(sender=__name__, sensor = self.sensor)
